...
const useGltfLoader = (source) => suspendReact.suspend(() => __awaiter(void 0, void 0, void 0, function* () {
    if (source instanceof Blob) {
...
function buildFallback(nodes, transform = new Transform()) {
    return (React__default["default"].createElement("group", null, Object.keys(nodes).map((key) => {
        ...
        if (node.type === 'Mesh') {
            return (React__default["default"].createElement("mesh", { castShadow: true, receiveShadow: true, key: node.name, scale: transform.scale, position: transform.position, rotation: transform.rotation, geometry: node.geometry, material: node.material, morphTargetInfluences: node.morphTargetInfluences || [] }));
        }
        return null;
    })));
}
...
const SpawnEffect = ({ onLoadedEffect, onLoadedEffectFinish }) => {
    ...
    const animationLoadedEffect = React.useMemo(() => __awaiter(void 0, void 0, void 0, function* () { return loadAnimationClip((onLoadedEffect === null || onLoadedEffect === void 0 ? void 0 : onLoadedEffect.animationSrc) || onLoadedEffect.src); }), [onLoadedEffect === null || onLoadedEffect === void 0 ? void 0 : onLoadedEffect.animationSrc, onLoadedEffect.src]);
...
const SpawnAnimation = ({ avatar, onLoadedAnimationFinish, onLoadedAnimation }) => {
    ...
    const animationClip = React.useMemo(() => __awaiter(void 0, void 0, void 0, function* () { return loadAnimationClip((onLoadedAnimation === null || onLoadedAnimation === void 0 ? void 0 : onLoadedAnimation.src) || ''); }), [onLoadedAnimation === null || onLoadedAnimation === void 0 ? void 0 : onLoadedAnimation.src]);
...
const Model = ({ scene, scale = 1, modelRef, onLoaded, onSpawnAnimationFinish, bloom }) => {
    ...
    React.useEffect(() => triggerCallback(onLoaded), [scene, materials, onLoaded]);
    React.useEffect(() => {
...
const StaticModel = ({ modelSrc, modelRef, scale = 1, setModelFallback, onLoaded, emotion, bloom }) => {
    ...
    return React__default["default"].createElement(Model, { modelRef: modelRef, scene: scene, scale: scale, onLoaded: onLoaded, bloom: bloom });
};
...
const BaseCanvas = ({ children = undefined, fov = 50, position = new three.Vector3(0, 0, 5), style, dpr = [window.devicePixelRatio * 0.5, 2], className }) => (React__default["default"].createElement(fiber.Canvas, { key: fov, className: `${styles$1['base-canvas']} ${className !== null && className !== void 0 ? className : ''}`, shadows: "soft", gl: { preserveDrawingBuffer: true, toneMappingExposure: 0.5, alpha: true }, dpr: dpr, camera: { fov, position }, resize: { scroll: true, debounce: { scroll: 50, resize: 0 } }, style: Object.assign(Object.assign({}, style), { background: 'transparent' }) }, children));
...
const Box = (_a) => {
    ...
    return (React__default["default"].createElement("mesh", Object.assign({ ref: ref, castShadow: true, receiveShadow: true }, baseProps),
        React__default["default"].createElement("boxBufferGeometry", null),
...
const Exhibit = ({ modelSrc, scale = 1.0, environment = 'city', position, style, className, shadows = false, float = false, fit = false, capture, snap = false, lockVertical = false, onLoaded, onLoading }) => {
    const model = React.useMemo(() => {
        ...
        if (!float) {
            return React__default["default"].createElement(StaticModelContainer, { modelSrc: modelSrc, scale: scale });
        }
        return React__default["default"].createElement(FloatingModelContainer, { modelSrc: modelSrc, scale: scale });
    ...
    return (React__default["default"].createElement(BaseCanvas, { position: position, style: style, className: className },
        React__default["default"].createElement(React.Suspense, { fallback: null },
            ...
            React__default["default"].createElement(drei.PresentationControls, { global: true, config: { mass: 2, tension: 500 }, snap: snap, rotation: [0, -0.3, 0], polar: lockVertical ? [0, 0] : [-Math.PI / 3, Math.PI / 3], azimuth: [-Infinity, Infinity] }, model && (React__default["default"].createElement(drei.Bounds, { fit: fit, clip: fit, observe: fit },
            ...
            React__default["default"].createElement(Environment, { environment: environment })),
...
const AnimationModel = ({ modelSrc, animationSrc, rotation = 20 * (Math.PI / 180), scale = 1, idleRotation = false, setModelFallback, onLoaded, headMovement = false, bloom }) => {
    ...
    const { scene } = useGltfLoader(modelSrc);
    const { nodes } = fiber.useGraph(scene);
    const animationClip = React.useMemo(() => __awaiter(void 0, void 0, void 0, function* () { return loadAnimationClip(animationSrc); }), [animationSrc]);
    ...
    useFallback(nodes, setModelFallback);
    return (React__default["default"].createElement(Model, { modelRef: ref, scene: scene, scale: scale, onLoaded: () => onLoaded && onLoaded(ref.current), onSpawnAnimationFinish: onSpawnAnimationFinish, bloom: bloom }));
};
...
const HalfBodyModel = ({ modelSrc, scale = 1, rotation = 20 * (Math.PI / 180), idleRotation = false, emotion, setModelFallback, onLoaded, headMovement = false, bloom }) => {
    ...
    useEmotion(nodes, emotion);
    useFallback(nodes, setModelFallback);
    return React__default["default"].createElement(Model, { modelRef: ref, scene: scene, scale: scale, onLoaded: onLoaded, bloom: bloom });
};
...
cameraTarget, cameraInitialDistance, cameraZoomTarget, headScale = 1, ambientLightColor, ambientLightIntensity, dirLightPosition, dirLightColor, dirLightIntensity, spotLightPosition, spotLightColor, spotLightAngle, spotLightIntensity, controlsMinDistance = 0.4, controlsMaxDistance = 2.5, updateCameraTargetOnZoom = false }) => {
    ...
    const { camera, gl, scene } = fiber.useThree();
    ...
    React.useEffect(() => {
        ...
        controls = new threeStdlib.OrbitControls(camera, gl.domElement);
        if (onCameraReady)
    ...
    React.useEffect(() => {
        if (!scene.getObjectByName('back-highlight')) {
            ...
            const spotLight = new three.SpotLight(spotLightColor, spotLightIntensity, 0, spotLightAngle, 0, 1);
...
const Shadow = () => (React__default["default"].createElement("group", { position: [0, 0, 0] },
    React__default["default"].createElement("mesh", { key: "shadow-catcher", receiveShadow: true, position: [0, 0, 0], "rotation-x": -Math.PI / 2 },
        React__default["default"].createElement("planeGeometry", { attach: "geometry", args: [5, 5] }),
        React__default["default"].createElement("shadowMaterial", { attach: "material", transparent: true, opacity: 0.2 }))));
...
modelSrc, animationSrc = undefined, poseSrc = undefined, environment = 'city', halfBody = false, shadows = false, scale = 1, ambientLightColor = '#fff5b6', ambientLightIntensity = 0.25, dirLightPosition = new three.Vector3(-3, 5, -5), dirLightColor = '#002aff', dirLightIntensity = 5, spotLightPosition = new three.Vector3(12, 10, 7.5), spotLightColor = '#fff5b6', spotLightAngle = 0.314, spotLightIntensity = 1, cameraTarget = CAMERA.TARGET.FULL_BODY.MALE, cameraInitialDistance = CAMERA.INITIAL_DISTANCE.FULL_BODY, style, emotion, idleRotation = false, capture, background, onLoaded, onLoading, dpr, className, headMovement = false, cameraZoomTarget = CAMERA.CONTROLS.FULL_BODY.ZOOM_TARGET, bloom, onLoadedEffect, onLoadedAnimation, children, effects, fov = 50 }) => {
    ...
    const AvatarModel = React.useMemo(() => {
        ...
        if (!!animationSrc && !halfBody && isValidFormat(animationSrc)) {
            return (React__default["default"].createElement(AnimationModelContainer, { modelSrc: modelSrc, animationSrc: animationSrc, scale: scale, idleRotation: idleRotation, onLoaded: onLoaded, headMovement: headMovement, bloom: bloom }));
        }
        if (halfBody) {
            return (React__default["default"].createElement(HalfBodyModelContainer, { emotion: emotion, modelSrc: modelSrc, scale: scale, idleRotation: idleRotation, onLoaded: onLoaded, headMovement: headMovement, bloom: bloom }));
        }
        if (isValidFormat(poseSrc)) {
            return (React__default["default"].createElement(PoseModelContainer, { emotion: emotion, modelSrc: modelSrc, scale: scale, poseSrc: poseSrc, onLoaded: onLoaded, bloom: bloom }));
        }
        return React__default["default"].createElement(StaticModelContainer, { modelSrc: modelSrc, scale: scale, onLoaded: onLoaded, emotion: emotion, bloom: bloom });
...
const FloorReflection = (_a) => {
    ...
    return (React__default["default"].createElement(React__default["default"].Fragment, null,
        React__default["default"].createElement("fog", { attach: "fog", args: [color, 4, 8] }),
